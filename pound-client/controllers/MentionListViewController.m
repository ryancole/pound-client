//
//  MentionListViewController.m
//  pound-client
//
//  Created by Ryan Cole on 1/3/13.
//  Copyright (c) 2013 Ryan Cole. All rights reserved.
//

#import "MentionListViewController.h"
...
@interface MentionListViewController () <UITableViewDataSource, UITableViewDelegate, SSPullToRefreshViewDelegate>
...
@implementation MentionListViewController
...
- (void)fetchMessages {
    ...
    if (_messages.count == 0) {
        ...
        [[APIClient sharedInstance] getMentions:^(NSMutableArray *mentions) {
        ...
        // get relative mentions
        [[APIClient sharedInstance] getMentionsSince:recent.id
                                             success:^(NSMutableArray *mentions) {
                                                 
                                                 if (mentions.count > 0) {
                                                     
                                                     // record the new messages
                                                     [_messages insertObjects:mentions atIndexes:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, mentions.count)]];
